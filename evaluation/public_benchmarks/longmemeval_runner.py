"""Retrieval-only LongMemEval diagnostic for Research memory systems."""
...
def run_memory_retrieval(dataset, output_dir, top_k=5, embedding_provider=None, limit=None):
    ...
    mode_rows = {"recent_window": [], "longterm_memory": []}
    predictions = []
    for case in cases:
        ...
        mode_rows["longterm_memory"].append(_row(case, retrieved_ids, relevant, latency_ms))
        predictions.append({"case_id": case.case_id, "question_type": case.metadata.get("question_type"), "evidence_ids": relevant, "recent_window": recent_ids, "longterm_memory": retrieved_ids})
    return {
